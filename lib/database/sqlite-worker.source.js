...
async function handle(req) {
  switch (req.type) {
    ...
    case "importDb": {
      ...
      // Close ALL open connections before import — OPFS handles from open
      // connections occupy pool capacity and can cause "No available handles
      // to import to" if exhausted. Closing all ensures we always have space.
      console.log(`[worker] importDb closing ${conns.size} existing connections`)
      for (const [key, db] of conns) {
      ...
      // Retry importDb with backoff — OPFS handles from a previous
      // (crashed/terminated) worker may still be held by the browser.
      const MAX_RETRIES = 5
      let lastError = null
      for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
        try {
          ...
          const isOpfsLockError =
            err?.name === "NoModificationAllowedError" ||
            String(err?.message ?? "").includes("createSyncAccessHandle") ||
            String(err?.message ?? "").includes("No available handles")
          if (isOpfsLockError && attempt < MAX_RETRIES) {
            const delay = attempt * 500
            console.warn(
              `[worker] importDb: OPFS lock, retry ${attempt}/${MAX_RETRIES} in ${delay}ms...`
            )
            await new Promise((r) => setTimeout(r, delay))
            continue
          }
