...
| AppShell | Shell compartilhado | `apps/web/features/navigation/components/app-shell.tsx` | Próprio + `AppDock`, `CommandPalette`, `ViewContainer` | Sem props; hospeda dock, command palette e view container | Loading do `useWorkspaceMode`, atalhos `mod+k`/`mod+o`, navegação por URL | `/` (`app/page.tsx`), `/notes`, `/highlights` | Reutilizar como shell único das três rotas canônicas; não duplicar dock/palette por página. |
...
| HighlightsMasterDetail | Composição de domínio | `apps/web/features/highlights/components/highlights-master-detail.tsx` | Próprio + `HighlightsFilterBar`, `HighlightEditDialog`, `useAllHighlights` | `active?`, `initialQuery?`, `onClose?`, `onNavigate?`; exports `buildNavigation`, `referenceLine` | Rail resumido + canvas com trecho Lora, "Abrir no leitor" primário, editar/copiar/excluir, Undo, vazio/loading/erro, lista→detalhe mobile | `ViewContainer`, `/highlights` | Reutilizar como superfície canônica de destaques; exports puros testáveis. |
...
| Shell principal | `AppShell` em `/`, `/notes` e `/highlights` | `AppDock`, `CommandPalette`, `ViewContainer` | URL de cada rota determina a view ativa; dock e palette são compartilhados | Leitura, Notas (master-detail `NotesWorkspace`), Destaques (browser); estados loading e atalhos globais |
