...
import { COLOR_LABELS, COLOR_SWATCHES, type ThemeColor, type ThemePalette } from "@/lib/theme"
...
export function ReaderHeader({
  ...
  const themeConfigContent = (
    <div className="space-y-6 py-4">
      ...
      <div className="space-y-3">
        ...
        <p className="text-xs text-muted-foreground">Escolha a paleta de cores base para o leitor.</p>
      ...
      <div className="space-y-3 pt-4 border-t border-border">
        ...
        <p className="text-xs text-muted-foreground">Escolha o tema visual do leitor.</p>
        <div className="grid grid-cols-3 gap-3">
          {[
            { value: "light" as const, label: "Claro", icon: <Sun className="h-4 w-4" /> },
            { value: "system" as const, label: "Sistema", icon: <Monitor className="h-4 w-4" /> },
            { value: "dark" as const, label: "Escuro", icon: <Moon className="h-4 w-4" /> },
          ].map((m) => {
      ...
      {palette === "default" && (
        <div className="space-y-3 pt-4 border-t border-border animate-in fade-in-50 duration-200">
          <h3 className="text-sm font-semibold leading-none">Cor de destaque</h3>
          <p className="text-xs text-muted-foreground">Escolha a cor principal dos destaques e botões.</p>
            ...
            {colorList.map((c) => {
              ...
              return (
                ...
                >
                  ...
                  {active && <Check className="h-3.5 w-3.5 text-primary shrink-0 ml-auto" />}
                </button>
              )
            })}
          </div>
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="flex items-center gap-3">
          ...
          <Slider
            ...
            step={2}
            value={[fontSize]}
            onValueChange={(val) => onChangeFontSize(Array.isArray(val) ? val[0] : val)}
            className="flex-1"
          />
      ...
      <div className="flex flex-col gap-2">
        <div className="flex justify-between items-center text-xs font-medium">
          ...
          <span className="text-muted-foreground">
            {verseSpacing === "small" ? "Compacto" : verseSpacing === "medium" ? "Padrão" : "Espaçoso"}
          </span>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-[10px] text-muted-foreground font-medium">Compacto</span>
          <Slider
            min={1}
            max={3}
            step={1}
            value={[verseSpacing === "small" ? 1 : verseSpacing === "medium" ? 2 : 3]}
            onValueChange={(val) => {
            ...
            className="flex-1"
          />
          <span className="text-[10px] text-muted-foreground font-medium">Espaçoso</span>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-2 border-t border-border pt-4 mt-1">
        <div className="flex justify-between items-center text-xs font-medium">
          ...
          <span className="text-muted-foreground">
            {readerMode === "narrow" ? "Estreito" : readerMode === "medium" ? "Padrão" : "Largo"}
          </span>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-[10px] text-muted-foreground font-medium">Estreito</span>
          <Slider
            min={1}
            max={3}
            step={1}
            value={[readerMode === "narrow" ? 1 : readerMode === "medium" ? 2 : 3]}
            onValueChange={(val) => {
              const numericVal = Array.isArray(val) ? val[0] : val
            ...
            className="flex-1"
          />
          <span className="text-[10px] text-muted-foreground font-medium">Largo</span>
        </div>
        <span className="text-[10px] text-muted-foreground leading-normal mt-1">
          Limita a largura máxima das margens do texto para tornar os parágrafos mais legíveis.
        </span>
      </div>
        ...
        <div className="grid grid-cols-3 gap-2">
          ...
          <button
            ...
            className={cn(
              ...
              mode === "light" ? "border-primary bg-primary/5" : "border-border hover:border-foreground/30 bg-card"
            )}
          >
            ...
            <span className="absolute bottom-1 right-2 z-10 text-[9px] font-bold text-neutral-800">Claro</span>
          </button>
          ...
          <button
            ...
            className={cn(
              "relative flex flex-col items-start gap-1 p-2 rounded-lg border-2 text-left transition-all overflow-hidden h-14 cursor-pointer",
              mode === "dark" ? "border-primary bg-primary/5" : "border-border hover:border-foreground/30 bg-card"
            )}
          >
            ...
            <span className="absolute bottom-1 right-2 z-10 text-[9px] font-bold text-white">Escuro</span>
          </button>
          ...
          <button
            ...
            className={cn(
              "relative flex flex-col items-start gap-1 p-2 rounded-lg border-2 text-left transition-all overflow-hidden h-14 cursor-pointer",
              mode === "system" ? "border-primary bg-primary/5" : "border-border hover:border-foreground/30 bg-card"
            )}
          >
            ...
            <span className="absolute bottom-1 right-2 z-10 text-[9px] font-bold mix-blend-difference text-white">Auto</span>
          </button>
        </div>
      </div>
  ...
  return (
    <>
      ...
      <div
        className={`sticky top-0 z-20 bg-background/95 backdrop-blur flex items-center justify-between pb-3 pt-3 px-4 border-b border-border min-h-[57px] transition-all duration-300 ease-in-out md:translate-y-0 md:opacity-100 ${showMiniReference
      ...
      >
        ...
        <div
          className={`absolute left-1/2 -translate-x-1/2 transition-all duration-300 md:hidden font-serif text-sm font-semibold text-foreground ${showMiniReference ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-1 pointer-events-none"
            }`}
        >
          {book.name} {chapter}
        </div>
        ...
        <div className="hidden md:flex items-center gap-2">
          ...
          <Popover>
            ...
            <PopoverContent className="w-80 p-5 flex flex-col gap-5" align="end">
              <div className="flex flex-col gap-1">
                <h4 className="font-semibold text-sm leading-none">Ajustes de exibição</h4>
                <p className="text-xs text-muted-foreground">Personalize sua experiência de leitura.</p>
              </div>
      ...
      {settingsOpen && !isDesktop && (
        <BottomSheet open={settingsOpen} onClose={() => setSettingsOpen(false)}>
          <div className="px-4 py-3 border-b border-border">
            <p className="text-sm font-semibold text-foreground">Ajustes de exibição</p>
          </div>
          <div className="p-5">
            {displaySettingsContent}
          </div>
