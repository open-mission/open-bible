...
import {
  BookOpen, FileText, Highlighter, X, ChevronRight, ChevronLeft,
  Settings, Sun, Moon, Plus, Link2, Link2Off, Trash2, ArrowLeft,
} from "lucide-react"
...
export function Sidebar({
    ...
    <div className="flex h-full flex-col bg-sidebar">
      ...
      <div className="flex items-center border-b border-sidebar-border shrink-0">
        {(["bible", "highlights", "notes"] as SidebarTab[]).map((t) => {
          const icon =
            t === "bible" ? <BookOpen className="h-3.5 w-3.5" /> :
            t === "highlights" ? <Highlighter className="h-3.5 w-3.5" /> :
            <FileText className="h-3.5 w-3.5" />
          const label = t === "bible" ? "Bíblia" : t === "highlights" ? "Destaques" : "Notas"
          const count = t === "highlights" ? highlights.length : t === "notes" ? notes.length : 0
          return (
            <button
              key={t}
              onClick={() => { setTab(t); if (t === "notes") setNotesView("list") }}
              aria-selected={tab === t}
              role="tab"
              className={`flex flex-1 items-center justify-center gap-1 py-2.5 text-xs font-medium transition-colors border-b-2 ${
                tab === t
        ...
        {tab === "bible" && (
          <>
            {biblePane === "books" || !selectedBookId ? (
              <BookList selectedBookId={selectedBookId} onSelectBook={handleSelectBook} />
            ) : (
              <ChapterGrid
              ...
              <ul className="divide-y divide-border">
                {highlightsWithMeta.map((item) => {
                  ...
                  return (
                    <li key={highlight.id}>
                      <button
                        onClick={() => { onJumpTo(book.id, chapter); onClose() }}
          ...
          <div className="flex flex-col flex-1 min-h-0">
            ...
            {notesWithMeta.length === 0 ? (
              <div className="flex flex-col items-center justify-center flex-1 gap-3 px-6 py-12">
                ...
                <p className="text-center text-sm text-muted-foreground leading-relaxed">
                  Nenhuma nota ainda. Clique em um versículo ou em &quot;Nova nota&quot; para começar.
                </p>
              </div>
            ) : (
              <ul className="flex-1 overflow-y-auto divide-y divide-border">
                {notesWithMeta.map(({ note, firstMeta }) => {
                  const verseHighlight = note.verseIds.length > 0
                      ...
                      >
                        <div className="flex items-center justify-between mb-1">
                          <div className="flex items-center gap-1.5 min-w-0">
                            {hlHex && (
                              <span className="inline-block w-2 h-2 rounded-full shrink-0" style={{ backgroundColor: hlHex }} />
                            )}
                            {firstMeta ? (
        ...
        {tab === "notes" && notesView === "detail" && activeNote && (
          <div className="flex flex-col flex-1 min-h-0">
              ...
              <div className="shrink-0 border-b border-border px-3 py-2 space-y-1">
                ...
                {activeNote.verseIds.map((vId) => {
                  ...
                  return (
                    <button
                      key={vId}
                      onClick={() => { onJumpTo(meta.book.id, meta.chapter); onClose() }}
            ...
            <div className="flex justify-end border-t border-border px-4 py-2 shrink-0 gap-2">
              <span className="text-xs text-muted-foreground/50 self-center hidden sm:inline">Ctrl+S</span>
              <button
                onClick={handleSaveEdit}
                disabled={!editNoteContent.trim() || editNoteContent.trim() === activeNote.content}
          ...
          <div className="flex flex-col flex-1 min-h-0">
            ...
            <div className="shrink-0 border-b border-border px-3 py-2">
              <p className="text-xs text-muted-foreground/70 italic">
                Escreva sua nota. Para vincular versículos, selecione-os no leitor e use o botão &quot;Nota&quot;.
              </p>
            </div>
  ...
  return (
    <>
      {/* Desktop sidebar */}
      <aside className={`hidden md:flex shrink-0 border-r border-border h-full flex-col overflow-hidden transition-all duration-200 ${sidebarCollapsed ? "w-0 border-0 overflow-hidden" : "w-64"}`}>
        {content}
      </aside>
