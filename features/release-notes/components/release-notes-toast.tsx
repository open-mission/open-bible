      ...
      <div className="fixed inset-x-0 bottom-0 md:inset-x-auto md:bottom-4 md:right-4 z-[90] p-4 md:p-0 pointer-events-none animate-in slide-in-from-bottom duration-300">
        <Card size="sm" className="pointer-events-auto w-full md:w-80 border border-border bg-popover/95 text-popover-foreground shadow-lg backdrop-blur-md rounded-t-2xl md:rounded-xl">
          <CardHeader className="flex flex-row items-center gap-2.5 p-4 pb-2 space-y-0">
            <span className={`flex size-8 shrink-0 items-center justify-center rounded-full transition-colors ${installed ? "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400" : "bg-primary/10 text-primary"}`}>
              {installed ? (
                <CheckCircle className="size-4 animate-in zoom-in-50 duration-300" />
              ) : (
                <IconSparkles className="size-4" />
              )}
            </span>
            <CardTitle className="text-sm font-semibold tracking-tight leading-none">
              {installed ? "Nova versão instalada" : (hasAppUpdate ? `Nova versão v${latestVersion} disponível` : "Nova versão disponível")}
            </CardTitle>
          </CardHeader>
          ...
          <CardFooter className="flex items-center justify-end gap-2 p-4 pt-2">
            <Button variant="ghost" size="sm" onClick={handleDismiss} className="text-xs h-8 cursor-pointer" disabled={installed}>
              Agora não
            </Button>
            {hasAppUpdate && (
              <Button variant="outline" size="sm" onClick={handleVerMudancas} className="text-xs h-8 cursor-pointer" disabled={installed}>
                Ver mudanças
              </Button>
            )}
            {hasPwaUpdate && (
              <Button size="sm" onClick={handleUpdate} className="text-xs h-8 cursor-pointer" disabled={installed}>
                {installed ? "Instalando..." : "Atualizar"}
              </Button>
            )}
          </CardFooter>
        </Card>
