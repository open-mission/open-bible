...
describe("tauri.conf.json", () => {
  ...
  it("AC-003: frontendDist e CSP para wasm e API", () => {
    const raw = readFileSync(join(root, "apps/desktop-tauri/tauri.conf.json"), "utf-8")
    const cfg = JSON.parse(raw)
    expect(cfg.build.frontendDist).toBe("../web/out")
    expect(cfg.build.beforeBuildCommand).toBe("node ../../scripts/build-tauri.mjs")
