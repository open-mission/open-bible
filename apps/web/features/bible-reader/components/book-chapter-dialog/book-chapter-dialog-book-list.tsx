  ...
  return (
    ...
    >
      <div className="flex flex-col gap-0.5">
        ...
        <span className={`text-[10px] uppercase tracking-wider font-semibold ${isSelected ? "text-primary/75" : "text-muted-foreground/75"}`}>
          {book.abbreviation}
        </span>
      </div>
      <ArrowRight className={`h-4 w-4 transition-transform duration-200 ${isSelected ? "text-primary translate-x-0.5" : "text-muted-foreground/50 group-hover:translate-x-0.5"}`} />
    </button>
  )
}
